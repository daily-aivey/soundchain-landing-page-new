    ...
    setTimeout(() => {
      ...
      revealElements.forEach((element, index) => {
        ...
        if (element.tagName.toLowerCase() === 'footer') {
          ...
          // PRODUCTION FIX: Desktop title uses standard observer with time-based blocking
          console.log('     🖥️ → DESKTOP TITLE (standard observer + time blocking)');
          regularElements.push(element);
          try {
            observer.observe(htmlElement);
            console.log(`     ✅ Successfully observing ${elementName} (desktop title with time blocking)`);
          } catch (err) {
