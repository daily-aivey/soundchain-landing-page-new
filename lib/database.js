...
class StorageAdapter {
  ...
  async addEmail(email, ip = null) { throw new Error('Not implemented'); }
...
class PostgreSQLAdapter extends StorageAdapter {
  ...
  async createTable() {
    if (!this.pool?.query) {
      console.error('createTable: pool/query missing');
      return;
    }
    try {
      ...
      `);
      await this.pool.query(`CREATE INDEX IF NOT EXISTS idx_signups_email ON signups(email)`);
    } catch (e) {
  ...
  async getCount() {
    await this.ready;
    if (!this.pool?.query) {
      console.error('getCount: pool/query missing');
      return 0;
    }
  ...
  async addEmail(email, ip = null) {
    await this.ready;
    if (!this.pool?.query) {
      console.error('addEmail: pool/query missing');
      return false;
    }
    try {
      await this.pool.query(
        'INSERT INTO signups (email, ip) VALUES ($1, $2)',
  ...
  async emailExists(email) {
    await this.ready;
    if (!this.pool?.query) {
      console.error('emailExists: pool/query missing');
      return false;
    }
...
export async function addSignup(email, ip = null) {
  const adapter = getStorageAdapter();
  return await adapter.addEmail(email, ip);
}
